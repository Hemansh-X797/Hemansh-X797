...
streak-stats: those are shared public instances that go down (503), run out
of quota (402), or time out. These are files in this repo, so they render
as long as GitHub renders.
...
data/projects.json, as <out>/card-<repo>-{dark,light}.svg.

Star and fork counts come from the live API on every run. Descriptions come
from projects.json when set, otherwise the repo's own GitHub description.

A token in $GITHUB_TOKEN unlocks the contribution and streak numbers (they
need the GraphQL API). Without one the card still renders, minus those
three tiles. If a repo in projects.json is private or doesn't exist yet
(pushed later), it's skipped with a note instead of crashing the run.
"""
...
# luxury: near-black / off-white / emerald / champagne gold
THEMES = {
    "dark": {
        "bg": "#0A0A0B", "border": "rgba(212,175,55,0.22)", "title": "#F5F5F4",
        "text": "#C9C9CC", "muted": "#8B8B90", "value": "#F5F5F4",
        "accent": "#10B981", "gold": "#D4AF37",
    },
    "light": {
        "bg": "#FFFFFF", "border": "rgba(166,124,0,0.28)", "title": "#101012",
        "text": "#3A3A3D", "muted": "#6B6B70", "value": "#101012",
        "accent": "#059669", "gold": "#A67C00",
    },
}

LANG_COLOR = {
    "JavaScript": "#f1e05a", "TypeScript": "#3178c6", "Python": "#3572A5",
    "HTML": "#e34c26", "CSS": "#563d7c", "C++": "#f34b7d", "C": "#a8b9cc",
    "Java": "#b07219", "Go": "#00ADD8", "Rust": "#dea584", "Shell": "#89e051",
    "Dart": "#00B4AB", "Vue": "#41b883", "Ruby": "#701516", "PHP": "#4F5D95",
...
def fetch_contributions(user: str, token: str | None):
    ...
    longest = run = 0
    for _, cnt in days:
        run = run + 1 if cnt > 0 else 0
        longest = max(longest, run)

    current = 0
    for date, cnt in reversed(days):
        if cnt > 0:
            current += 1
        elif date != days[-1][0]:
            break
...
def render_private_repo(entry, theme):
...
def main(argv=None):
    ...
    for entry in wanted:
        ...
        if not src:
            # not visible with this token -- either private or not pushed yet.
            # render an honest "private" card instead of skipping silently or
            # inventing star/fork numbers for a repo we cannot see.
            for theme in ("dark", "light"):
                dest = args.out / f"card-{entry['repo']}-{theme}.svg"
                dest.write_text(render_private_repo(entry, theme), encoding="utf-8")
            print(f"  !! {entry['repo']} not visible to this token -- "
                  f"wrote a private-repo card instead of live stats")
            continue
        card = {
            "name": src["name"],
